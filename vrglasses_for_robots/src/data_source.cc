...
#include <vrglasses_for_robots/data_source.h>
...
void DataSource::getPositions(cv::Mat& positions) {
  ...
  for (size_t i = 0; i < data_.size(); i++) {
    Eigen::Matrix<double, 3, 1>& pos = data_[i].T_WC.getPosition();
    positions.at<float>(i, 0) = (float)pos(0);
    positions.at<float>(i, 1) = (float)pos(1);
    positions.at<float>(i, 2) = (float)pos(2);
  }
}
